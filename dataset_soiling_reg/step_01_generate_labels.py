"""
Soiling Label GeoJSON Generator for Regression Task

Generates georeferenced GeoJSON label files for soiling prediction model training.
Each GeoJSON contains a single point feature with the normalized soiling index value
as a property, centered on the device geometry in the native UTM projection.

Output:
- labels/*.geojson — point features with soiling_normalized property
...
OUTPUT_DIR = "dataset_soiling_reg/data"
LABELS_SUBDIR = "labels"
METADATA_SUBDIR = "metadata"

CHIP_SIZE = 32                  # pixels (for metadata reference)
...
def generate_filename(plant: str, device: str, date: pd.Timestamp) -> str:
    """Generate clean filename for label GeoJSON."""
    ...
    return f"{plant_clean}_{device_clean}_{date_str}.geojson"


def save_label_geojson(
    filepath: Path,
    value: float,
    utm_x: float,
    utm_y: float,
    ...
    """Save label as GeoJSON point feature with soiling value as property.
    
    Args:
        filepath: path to save the GeoJSON file
        value: normalized soiling value (0-1)
        utm_x: UTM x coordinate of the point
        utm_y: UTM y coordinate of the point
        epsg: EPSG code for the UTM zone
        additional_properties: optional additional properties to include
    """
    properties = {
        "soiling_normalized": value,
    }
    ...
    feature_collection = {
        ...
        "features": [
            {
                "type": "Feature",
                "geometry": mapping(Point(utm_x, utm_y)),
...
def process_row(
    ...
    """Process a single row and generate label GeoJSON."""
    try:
        # Extract values
        plant = row["ID_PLANT"]
        ...
        # Compute UTM bbox (for metadata reference)
        bbox = compute_utm_bbox(utm_x, utm_y, chip_size, resolution)
        
        # Generate filename and save GeoJSON
        filename = generate_filename(plant, device, date)
        filepath = labels_dir / filename
        
        # Save with additional properties for reference (optional)
        save_label_geojson(
            filepath=filepath,
            value=soiling_normalized,
            utm_x=utm_x,
            utm_y=utm_y,
            epsg=utm_epsg,
...
def print_summary(entries: list[dict], labels_dir: Path, failed_count: int):
    ...
    total_size = sum(f.stat().st_size for f in labels_dir.glob("*.geojson"))
    ...
    print("\n" + "=" * 60)
    print("LABEL GENERATION SUMMARY (GeoJSON for Regression)")
    print("=" * 60)
    print(f"  GeoJSONs generated:              {len(entries):>10,}")
    print(f"  Failed rows:                     {failed_count:>10,}")
    print(f"  Output directory:                {labels_dir}")
    print(f"  Total size:                      {size_kb:>10.2f} KB")
    print()
    ...
    print(f"  Normalized value std:            {np.std(norm_values):>10.4f}")
    print()
...
def main():
    logger.info("Starting soiling label GeoJSON generation for regression task")
