...
Ingests local PASTIS-HD crop type label files into the rslearn dataset structure.
...
```bash
rslearn dataset materialize --root ./dataset_pastis --workers 32 \
  --retry-max-attempts 5 --retry-backoff-seconds 5 --ignore-errors
```

Note: make sure to have dataset_pastis/config.json
  ...
  "layers": {
    "label": {
      ...
      "data_source": {
        ...
        "src_dir": "../pastis_hd_target/TARGET",
        "path_template": "{name}_TARGET.tif",
        "ingest": true
      },
